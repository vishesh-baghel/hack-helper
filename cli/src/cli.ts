    ...
    try {
      ...
      const updateDisplay = () => {
        ...
        spinner.currentFrame =
          (spinner.currentFrame + 1) % spinner.frames.length;
      };
      ...
      const formattedResponse = result.data.messages[
        result.data.messages.length - 1
      ].content
        .replace(/\\n/g, "\n")
        .replace(/\\"([^\\"]*)\\"/, '"$1"'); // Clean up escaped quotes
      ...
      if (result.data.generatedFiles && result.data.generatedFiles.length > 0) {
        ...
        console.log(
          chalk.gray(
            "   The project structure is being created by the orchestratorAgent,"
          )
        );
        console.log(
          chalk.gray(
            "   which delegates to briefExtractorAgent and other specialized agents."
          )
        );
      }
    } catch (error) {
